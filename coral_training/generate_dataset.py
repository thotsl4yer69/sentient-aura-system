...
# ============================================================================
# PARTICLE GENERATION FROM DESCRIPTIONS
# ============================================================================

def parse_description_to_particles(description: str, num_particles: int = 10000) -> np.ndarray:
    """
    Parse pre-generated LLM description into particle positions.
    
    Args:
        description: LLM description of particle arrangement
        num_particles: Number of particles to generate
    
    Returns:
        np.ndarray: Particle positions (num_particles, 3)
    """
    import re
    ...
    percentages = re.findall(r'(\d+)%', description)
    ...
    # Check for tendrils/zones
    has_tendrils = 'TENDRILS' in description or 'tendrils' in description
    has_zones = 'ZONES' in description or 'zones' in description
    
    if has_tendrils and len(percentages) >= 3:
        tendril_pct = float(percentages[2]) / 100.0
    else:
        tendril_pct = 0.2 if has_tendrils else 0.0
    
    # Remaining goes to aura or zones
    remaining = 1.0 - core_pct - tendril_pct
    aura_pct = min(aura_pct, remaining)
    
    # Calculate particle counts
    n_core = int(num_particles * core_pct)
    n_aura = int(num_particles * aura_pct)
    n_tendril = int(num_particles * tendril_pct)
    n_remaining = num_particles - n_core - n_aura - n_tendril
    
    idx = 0
    
    # CORE: Tight sphere at center
    for i in range(n_core):
        theta = np.random.uniform(0, 2 * np.pi)
        phi = np.random.uniform(0, np.pi)
        r = np.random.uniform(0, 0.25)  # Tight core
        
        x = r * np.sin(phi) * np.cos(theta)
        y = r * np.sin(phi) * np.sin(theta) + 1.0  # Center at y=1.0
        z = r * np.cos(phi)
        
        positions[idx] = [x, y, z]
        idx += 1
    
    # AURA: Outer diffuse layer
    for i in range(n_aura):
        theta = np.random.uniform(0, 2 * np.pi)
        phi = np.random.uniform(0, np.pi)
        r = np.random.uniform(0.25, 0.6)  # Outer shell
    ...
    if n_tendril > 0:
        num_tendrils = 3  # Default 3 tendrils
        particles_per_tendril = n_tendril // num_tendrils
        
        for t in range(num_tendrils):
            # Each tendril in a different direction
            base_angle = (t / num_tendrils) * 2 * np.pi
            
            for i in range(particles_per_tendril):
                distance = np.random.uniform(0.3, 0.9)
                spread = np.random.normal(0, 0.1)
                
                x = distance * np.cos(base_angle + spread)
                y = 1.0 + np.random.uniform(-0.2, 0.2)
                z = distance * np.sin(base_angle + spread)
                
                if idx < num_particles:
                    positions[idx] = [x, y, z]
                    idx += 1
    
    # Fill remaining particles randomly in aura
...
def generate_training_dataset():
    """Generate complete training dataset from scenarios."""
    
    logger.info("=" * 70)
    logger.info("CORAL TPU TRAINING DATASET GENERATION")
    logger.info("=" * 70)
    logger.info(f"Scenarios: {len(SCENARIOS)}")
    logger.info(f"Features: 68 (rich)")
    ...
    for idx, (scenario_name, features) in enumerate(SCENARIOS.items()):
        ...
        description = PRE_GENERATED_LLM_DESCRIPTIONS.get(scenario_name, "")
        
        if not description:
            logger.warning(f"  No description found for {scenario_name}, using default")
            description = "CORE: 50% particles. AURA: 30% particles. TENDRILS: 20% particles."
        
        # Generate particle positions from description
    ...
    input_path = dataset_dir / f'inputs_rich_{timestamp}.npy'
    output_path = dataset_dir / f'outputs_rich_{timestamp}.npy'
    metadata_path = dataset_dir / f'metadata_rich_{timestamp}.json'
